...
controllerData.key= (req,res)=>{
    let view = async () =>{
        if(req.session.saveName){
                ...
                res.render('text',{admin:admin[0].Username,permission:permission,name:req.session.saveName,value:results[0].text,select:results[0].selected,key:req.params.key})
        }
        else{
            let results =await data.view(req.params.key);
            let admin = await data.selectUsername(req.params.key);
            res.render('text',{admin:admin[0].Username,permission:1,name:'',value:results[0].text,select:results[0].selected,key:req.params.key})
        }
    }
